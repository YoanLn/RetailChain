...
C'est un choix de modélisation qui est très important. On n'a pas utilisé la même stratégie pour toutes les dimensions, parce qu'elles n'ont pas les mêmes besoins.
...
* **Justification :** C'est vital pour l'intégrité des analyses.
    * **Exemple Client :** Si un client déménage de Paris à Lyon en 2023, on *doit* pouvoir lier ses achats de 2022 à Paris. Si on avait fait un SCD Type 1, toutes ses ventes de 2022 auraient été faussement attribuées à Lyon.
...
L'objectif était d'avoir un processus ETL fonctionnel et robuste. Tout est fait en **PL/pgSQL**.

### Couche Staging

* **Choix :** On a créé un schéma `staging` séparé pour accueillir les données brutes (`_raw`).
* **Justification :** C'est une bonne pratique fondamentale. Le DWH reste propre. L'ETL lit depuis `staging`, transforme, et charge dans `dwh`. Ça permet aussi de relancer l'ETL sans risque.
...
* **Chargement Dimensions (SCD) :**
    * Pour le SCD Type 1 (`dim_store`), on utilise `INSERT ... ON CONFLICT`.
...
* **Chargement des Faits (`fact_sales`) :**
    ...
    * **Jointure SCD :** Pour lier un fait à la *bonne version historique* de la dimension (client ou produit), on ne fait surtout pas un join sur `is_current = TRUE`.
...
3.  **Index BRIN (Avancé) :**
    ...
    * **Justification :** Un index B-Tree sur 100k+ lignes triées (comme les dates) est lourd. Un index **BRIN** est très léger (il stocke min/max par bloc). Comme nos données sont triées par date, il permet à PostgreSQL d'ignorer 99% de la table lors d'un filtre sur une plage de dates. Le coût est minime pour un gain énorme.
...
* **Justification :** C'est une optimisation majeure pour les analystes et les dashboards.
    ...
    * Les `marts` contiennent ces agrégats pré-calculés. Une requête sur `marts.mv_monthly_sales_country` est presque **instantanée**.
